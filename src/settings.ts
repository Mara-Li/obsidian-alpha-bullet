...
export class MarkdownListSortSettings extends PluginSettingTab {
	...
	async display(): Promise<void> {
		...
		- \`sml_glossary\` *(\`boolean\`)*: ${i18next.t("settings.group")}
		...
		new Setting(containerEl).setName(`${i18next.t("level")}`).addSlider((slider) =>
			slider
		...
		new Setting(containerEl).setName(`${i18next.t("sort")}`).addToggle((toggle) =>
		...
		new Setting(containerEl).setName(`${i18next.t("reverse")}`).addToggle((toggle) =>
		...
		new Setting(containerEl).setName(`${i18next.t("group")}`).addToggle((toggle) =>
